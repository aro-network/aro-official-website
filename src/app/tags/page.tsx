...
export default function TagClient() {
    ...
    try {
      ...
      if (isNumeric) {
        ...
        const tagRes = await axios.get(
          `https://wordpress.enreach.network/wp-json/wp/v2/tags/${tagId}`
        );
        ...
        const tagRes = await axios.get(
          `https://wordpress.enreach.network/wp-json/wp/v2/tags?slug=${slug}`
        );
      ...
      const postsRes = await axios.get(
        `https://wordpress.enreach.network/wp-json/wp/v2/posts?tags=${tagId}&_embed&per_page=5&page=${pageNum}`
      );
  ...
  return <div className=" bg-[url(/banner-bg.svg)] mo:bg-top bg-cover  z-[100000] relative  w-full h-screen">
    <AHeader />
    <div className=" w-container px-[5.125rem] mo:px-[1.0625rem] h-[calc(100vh-6.25rem)] mo:h-full pt-[5.0625rem] m-auto md:w-full mo:w-full ">
      ...
      <div className=" grid grid-cols-[repeat(auto-fill,minmax(24.75rem,1fr))] justify-between gap-5 mt-[5rem] mo:mt-[1.875rem]  ">
      ...
      {hasMore && <div className="w-full text-center pt-[3.75rem] pb-[5rem] mo: ">
