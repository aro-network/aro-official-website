export function VisionStatement() {
    return <div className="h-screen flex items-center justify-center lg:justify-end lg:h-[2000px]">
        <div className="w-full lg:max-w-[60%] sticky top-24 lg:top-60">
            <h2 className="text-center normal-case section-title lg:text-left">BRINGING INTELLIGENCE WITHIN</h2>
            <h2 className="text-center normal-case section-title lg:text-left"> YOUR REACH </h2>

            <p className="text-center vision-statement__content lg:text-left">
                We envision a future where intelligence is always within reach, delivering real-time responses and enabling boundless interactions at the edge, redefining your digital experiences.
            </p>
        </div>
    </div>
}
